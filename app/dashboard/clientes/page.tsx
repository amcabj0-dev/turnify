...
export default function Clientes() {
  ...
  const prefijo = (negocio?.pais_prefijo || '+54')
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        .cliente-card { background: #0f1628; border: 1px solid rgba(79,142,247,0.12); border-radius: 12px; padding: 13px; cursor: pointer; transition: border-color 0.2s, transform 0.15s; }
        ...
        .input-field { width: 100%; background: #090d1a; border: 1px solid rgba(79,142,247,0.2); border-radius: 10px; padding: 10px 12px; color: #e8edf8; font-family: 'DM Sans', sans-serif; font-size: 13px; outline: none; }
        ...
        .modal-sheet { background: #0f1628; border-radius: 20px 20px 0 0; border-top: 1px solid rgba(79,142,247,0.2); width: 100%; max-width: 480px; max-height: 88vh; overflow-y: auto; padding: 20px 16px 40px; }
        .historial-row { background: #090d1a; border: 1px solid rgba(79,142,247,0.08); border-radius: 10px; padding: 10px 12px; margin-bottom: 7px; }
        .badge { font-size: 10px; padding: 2px 8px; border-radius: 20px; font-weight: 600; }
        @keyframes slideUp { from { transform: translateY(100%); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
        .modal-sheet { animation: slideUp 0.25s ease; }
      `}</style>

      <div style={{ minHeight: '100vh', background: '#090d1a', color: '#e8edf8', fontFamily: "'DM Sans', sans-serif", paddingBottom: '80px' }}>

        {/* Header */}
        <div style={{ background: '#0f1628', borderBottom: '1px solid rgba(79,142,247,0.15)', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', position: 'sticky', top: 0, zIndex: 50 }}>
          <Link href="/dashboard" style={{ color: '#6b7fa3', display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ position: 'relative' }}>
            <svg style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#6b7fa3' }} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            {[
              ...
              <div key={i} style={{ background: '#0f1628', border: '1px solid rgba(79,142,247,0.12)', borderRadius: '12px', padding: '12px' }}>
          ...
          {clientesFiltrados.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#6b7fa3', fontSize: '13px' }}>
              ...
              <div key={cliente.id} className="cliente-card" onClick={() => abrirCliente(cliente)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '14px', fontWeight: 600, fontFamily: "'Syne', sans-serif" }}>{cliente.nombre}</div>
                    <div style={{ fontSize: '11px', color: '#6b7fa3', marginTop: '2px' }}>
                      {cliente.whatsapp ? prefijo + ' ' + cliente.whatsapp : 'Sin WhatsApp'}
                    </div>
                  </div>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6b7fa3" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                </div>

                <div style={{ display: 'flex', gap: '8px', marginTop: '10px', paddingTop: '10px', borderTop: '1px solid rgba(79,142,247,0.08)' }}>
                  <div style={{ flex: 1, textAlign: 'center' }}>
                    <div style={{ fontSize: '16px', fontWeight: 800, fontFamily: "'Syne', sans-serif", color: '#4f8ef7' }}>{cliente.cantidadTurnos}</div>
                    <div style={{ fontSize: '10px', color: '#6b7fa3' }}>turnos</div>
                  </div>
                  <div style={{ width: '1px', background: 'rgba(79,142,247,0.1)' }} />
                  <div style={{ flex: 1, textAlign: 'center' }}>
                    <div style={{ fontSize: '16px', fontWeight: 800, fontFamily: "'Syne', sans-serif", color: '#00e5a0' }}>
                      {'$' + cliente.totalGastado.toLocaleString('es-AR')}
                    </div>
                    <div style={{ fontSize: '10px', color: '#6b7fa3' }}>gastado</div>
                  </div>
                  <div style={{ width: '1px', background: 'rgba(79,142,247,0.1)' }} />
                  <div style={{ flex: 2, textAlign: 'center' }}>
                    <div style={{ fontSize: '11px', fontWeight: 600, color: '#00d4ff' }}>
                      {cliente.ultimoTurno
                        ? new Date(cliente.ultimoTurno.fecha_hora).toLocaleDateString('es-AR', { day: '2-digit', month: 'short', year: '2-digit' })
                        : '—'}
                    </div>
                    <div style={{ fontSize: '10px', color: '#6b7fa3' }}>último turno</div>
                  </div>
                </div>
              </div>
            ))
            ...
            {!editando ? (
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: '17px' }}>{clienteSeleccionado.nombre}</div>
                    <div style={{ fontSize: '12px', color: '#6b7fa3', marginTop: '2px' }}>
                      {clienteSeleccionado.whatsapp ? prefijo + ' ' + clienteSeleccionado.whatsapp : 'Sin WhatsApp'}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '16px' }}>
                  {[
                    ...
                    <div key={i} style={{ background: '#090d1a', border: '1px solid rgba(79,142,247,0.1)', borderRadius: '10px', padding: '10px', textAlign: 'center' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '16px' }}>
                  {clienteSeleccionado.whatsapp && (
                      ...
                      href={'https://wa.me/' + prefijo.replace('+', '') + clienteSeleccionado.whatsapp}
                      target="_blank"
                      rel="noreferrer"
                  ...
                  <button className="action-btn" onClick={() => setEditando(true)}
                ...
                {/* Historial de turnos */}
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: '13px', marginBottom: '10px' }}>
                  Historial de turnos
                </div>
                {loadingHistorial ? (
                  <div style={{ textAlign: 'center', padding: '20px', color: '#6b7fa3', fontSize: '12px' }}>Cargando...</div>
                ) : historial.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '20px', color: '#6b7fa3', fontSize: '12px' }}>Sin turnos registrados</div>
                ) : (
                  historial.map(t => (
                    <div key={t.id} className="historial-row">
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <div style={{ fontSize: '12px', fontWeight: 600 }}>{t.servicios?.nombre || 'Sin servicio'}</div>
                          <div style={{ fontSize: '11px', color: '#6b7fa3', marginTop: '2px' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          ...
                          {t.monto > 0 && (
                            <div style={{ fontSize: '11px', color: t.pagado ? '#00e5a0' : '#6b7fa3', marginTop: '4px' }}>
                ...
                {/* Eliminar */}
                {!confirmEliminar ? (
                  <button className="action-btn" onClick={() => setConfirmEliminar(true)}
                    style={{ width: '100%', marginTop: '16px', background: 'rgba(255,107,107,0.08)', color: '#ff6b6b', border: '1px solid rgba(255,107,107,0.2)' }}>
                    🗑️ Eliminar cliente
                  </button>
                ) : (
                  <div style={{ marginTop: '16px', background: 'rgba(255,107,107,0.08)', border: '1px solid rgba(255,107,107,0.25)', borderRadius: '12px', padding: '14px', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#6b7fa3', marginBottom: '12px' }}>Se eliminará el cliente pero sus turnos quedarán en el sistema.</div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button className="action-btn" onClick={() => setConfirmEliminar(false)}
                        style={{ flex: 1, background: 'rgba(79,142,247,0.12)', color: '#4f8ef7', border: '1px solid rgba(79,142,247,0.25)' }}>
                        Cancelar
                      </button>
                      <button className="action-btn" onClick={eliminarCliente}
                        style={{ flex: 1, background: 'rgba(255,107,107,0.2)', color: '#ff6b6b', border: '1px solid rgba(255,107,107,0.4)' }}>
                        Sí, eliminar
                      </button>
                    </div>
                  </div>
              ...
              /* Editar cliente */
              <>
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: '17px', marginBottom: '20px' }}>Editar cliente</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div>
                    <div style={{ fontSize: '11px', color: '#6b7fa3', marginBottom: '6px' }}>Nombre</div>
                    <input className="input-field" value={editNombre} onChange={e => setEditNombre(e.target.value)} placeholder="Nombre del cliente" />
                  </div>
                  <div>
                    <div style={{ fontSize: '11px', color: '#6b7fa3', marginBottom: '6px' }}>WhatsApp (sin prefijo, ej: 2804001234)</div>
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
                  <button className="action-btn" onClick={() => setEditando(false)}
                    style={{ flex: 1, background: 'rgba(79,142,247,0.08)', color: '#6b7fa3', border: '1px solid rgba(79,142,247,0.15)' }}>
                    Cancelar
                  </button>
                  <button className="action-btn" onClick={guardarEdicion} disabled={guardando}
                    style={{ flex: 2, background: 'linear-gradient(135deg,#4f8ef7,#00d4ff)', color: '#000', border: 'none', opacity: guardando ? 0.6 : 1 }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#0f1628', borderTop: '1px solid rgba(79,142,247,0.15)', display: 'flex', justifyContent: 'space-around', alignItems: 'center', padding: '10px 0 14px', zIndex: 100 }}>
        <Link href="/dashboard" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', color: '#6b7fa3', fontSize: '9px', fontFamily: "'DM Sans', sans-serif", textDecoration: 'none' }}>
        ...
        <Link href="/dashboard/empleados" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', color: '#6b7fa3', fontSize: '9px', fontFamily: "'DM Sans', sans-serif", textDecoration: 'none' }}>
